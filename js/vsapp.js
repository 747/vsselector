 ...
 * == VDOM models ==
 */
var BigBox, CharList, CharTab, CharTag, External, Header, I, PickChar, Picker, Row, Search, SearchBox, Social, TheApp, Toggler, UsefulBar, VResult, Workspace, elAbout, hint, isObject, messages, numlike, onMatch, pickerTab, popup, query, signboard, uiLang,
  indexOf = [].indexOf;
...
UsefulBar = {
  view: function() {
    return m('.notification.has-text-white.transparent', m('strong.has-text-black.has-text-weight-bold', I('very_useful_content')), m('a.tag.is-info', {
      txt: '竈門禰󠄀豆子　※「禰󠄀」は「ネ＋爾」が正しい表記となります。',
      onclick: m.withAttr('txt', signboard.ins)
    }, '竈門禰󠄀豆子'), ' ', m('a.tag.is-info', {
      txt: '煉󠄁獄杏寿郎　※「煉󠄁」は「火＋東」が正しい表記となります。',
      onclick: m.withAttr('txt', signboard.ins)
    }, '煉󠄁獄杏寿郎'), ' ', m('a.tag.is-info', {
      txt: '鬼舞辻󠄀無惨　※「辻󠄀」はしんにょうの点が1つの字が正しい表記となります。',
      onclick: m.withAttr('txt', signboard.ins)
    }, '鬼舞辻󠄀無惨'), ' ', m('a.tag.is-info', {
      txt: '鋼鐡塚蛍',
      onclick: m.withAttr('txt', signboard.ins)
    }, '(鋼鐡塚蛍)'));
  }
...
Workspace = {
  view: function() {
    return m('#workspace.columns.is-multiline.section.transparent', m('#useful.column.is-full', m(UsefulBar)), m('.columns.is-full', m('#viewer.column.is-7', m(BigBox), m('#breakdown.message.is-warning.is-fullwidth', m(CharList)), m(Social)), m(Picker)));
  }
};
  ...
  view: function(v) {
    ...
    return m('tr', {
      ...
      if (seq) {
        ...
        path = MISSING.indexOf(code) >= 0 ? `./images/te/supp/${code}.svg` : `./images/te/${code}.svg`;
        return [
          m('td',
          {
            colSpan: 2
          ...
          m('img.glyph',
          {
            src: (function() {
              switch (type) {
                ...
                case "emoji":
                  code = `${base ? base.toString(16) + '-' : ''}${id.toString(16)}`;
                  if (MISSING.indexOf(code) >= 0) {
                    return `./images/te/supp/${code}.svg`;
                  } else {
                    return `./images/te/${code}.svg`;
                  }
                  break;
                default:
